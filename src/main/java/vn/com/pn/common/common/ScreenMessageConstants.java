package vn.com.pn.common.common;

public class ScreenMessageConstants {
    public static final String SUCCESS = "API đã thực hiện thành công.";

    public static final String NOT_LOGIN = "Người dùng chưa đăng nhập.";

    public static final String INCORRECT_ROLE = "Người dùng không có quyền thực hiện API này.";

    public static final String FAILURE = "API đã thực hiện thất bại.";
}
